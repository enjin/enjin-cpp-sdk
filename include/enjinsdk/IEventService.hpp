...
public:
    ...
    virtual void set_connected_handler(std::function<void()> handler) = 0;

    /// \brief Sets a handler for when the service disconnects from the server.
    /// \param handler The handler.
    virtual void set_disconnected_handler(std::function<void()> handler) = 0;

    /// \brief Sets a handler for when the service encounters an error.
    /// \param handler The handler.
    virtual void set_error_handler(std::function<void(const std::exception& e)> handler) = 0;

    /// \brief Registers a event listener and provides the registration for it.
    /// \param listener The listener.
    /// \return The registration.
    virtual const EventListenerRegistration& register_listener(std::shared_ptr<IEventListener> listener) = 0;
    ...
    register_listener_including_types(std::shared_ptr<IEventListener> listener,
                                      std::vector<models::EventType> types) = 0;
    ...
    virtual void subscribe_to_project(std::string project) = 0;

    /// \brief Closes a channel for the specified project, preventing listeners from receiving events for it.
    /// \param project The project's UUID.
    virtual void unsubscribe_to_project(std::string project) = 0;
    ...
    [[nodiscard]] virtual bool is_subscribed_to_project(std::string project) const = 0;

    /// \brief Opens a channel for the specified player, allowing listeners to receive events for it.
    /// \param project The project's UUID.
    /// \param player The player ID.
    virtual void subscribe_to_player(std::string project, std::string player) = 0;

    /// \brief Closes a channel for the specified player, preventing listeners from receiving events for it.
    /// \param project The project's UUID.
    /// \param player The player ID.
    virtual void unsubscribe_to_player(std::string project, std::string player) = 0;

    /// \brief Determines if this service is subscribed to the specified channel.
    /// \param project The project's UUID.
    /// \param player The player ID.
    /// \return Whether this service is subscribed to the channel.
    [[nodiscard]] virtual bool is_subscribed_to_player(std::string project, std::string player) const = 0;

    /// \brief Opens a channel for the specified asset, allowing listeners to receive events for it.
    /// \param asset The asset ID.
    virtual void subscribe_to_asset(std::string asset) = 0;

    /// \brief Closes a channel for the specified asset, preventing listeners from receiving events for it.
    /// \param asset The asset ID.
    virtual void unsubscribe_to_asset(std::string asset) = 0;

    /// \brief Determines if this service is subscribed to the specified channel.
    /// \param asset The asset ID.
    /// \return Whether this service is subscribed to the channel.
    [[nodiscard]] virtual bool is_subscribed_to_asset(std::string asset) const = 0;

    /// \brief Opens a channel for the specified wallet, allowing listeners to receive events for it.
    /// \param wallet The wallet's address.
    virtual void subscribe_to_wallet(std::string wallet) = 0;

    /// \brief Closes a channel for the specified wallet, preventing listeners from receiving events for it.
    /// \param wallet The wallet's address.
    virtual void unsubscribe_to_wallet(std::string wallet) = 0;

    /// \brief Determines if this service is subscribed to the specified channel.
    /// \param wallet The address.
    /// \return Whether this service is subscribed to the channel.
    [[nodiscard]] virtual bool is_subscribed_to_wallet(std::string wallet) const = 0;
};
